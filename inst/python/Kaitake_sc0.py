...
os.environ['PROJ_LIB'] = "C:\\ProgramData\\Anaconda3\\envs\\proofofabsence\\Library\\share\\proj"
...
def main():
    ...
    inputDataPath = os.path.join(os.getenv('POFPROJDIR', default = '.'), 'inst', 'example_data', 'Kaitake_possums')
    if not os.path.isdir(inputDataPath):
        os.mkdir(inputDataPath)

    outputDataPath = os.path.join(os.getenv('POFPROJDIR', default = '.'), 'results')
...
#    myParams.setPuLogistic(rmax_pstar, ki, ku, pind, p_disperse, pu)
    ...
    myParams.setPrior(0.6, 0.8, 0.85)
