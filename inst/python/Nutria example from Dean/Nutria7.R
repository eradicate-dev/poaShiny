...
# def main():
    ...
    ##   <<< COMPULSORY >>>
    inputDataPath = os$path$join("inst", "python", "Nutria example from Dean", 'poa',
            'Nutria', 'Data')
    
        
    ##   <<< COMPULSORY >>>
    outputDataPath = os$path$join("inst", "python", "Nutria example from Dean", 'poa',
            'Nutria', 'Results', 'mod7')
...
#    zoneShapeFName = os.path.join(inputDataPath, 'noZoneTestNutria.shp')  # 'mngtZonesPu_1.shp')     #'mngtZone_HalfCellGrow.shp')    # 'mngtZonesPu_3.shp')
    ...
    firstRun = bi$bool("True")        # True or False

    ##   <<< COMPULSORY >>>
    # resolution for analysis
    Resolution = bi$float(100.0)

    ##   <<< COMPULSORY >>>
    # EPSG - PROJECTION SYSTEM
    epsg = bi$int(26918)    # "NAD_1983_UTM_Zone_18N"
   
    ##   <<< OPTIONAL >>>
    #############################################################################
    ##### ADD SURVEILLANCE TYPES                                                #
    animals = params$AnimalTypes()                                              #
    TYPE_LANDPLATFORM = bi$int(12) # index number for land platform                     #
    animals$addAnimal(TYPE_LANDPLATFORM, "LandPlatform", params$DETECT_ANIMAL)  #
    TYPE_WATERPLATFORM = bi$int(13) # index number for WATER PLATFORM                   #
    animals$addAnimal(TYPE_WATERPLATFORM, "WaterPlatform", params$DETECT_ANIMAL)#
    TYPE_CAMERA = bi$int(14) # index number for Camera traps                            #
    animals$addAnimal(TYPE_CAMERA, "Camera", params$DETECT_ANIMAL)              #
    TYPE_DOGTRACK = bi$int(15) # index number for DOG ONLY TRACKS                       #
    animals$addAnimal(TYPE_DOGTRACK, "DogTrack", params$DETECT_ANIMAL)          #
    TYPE_GROUND = bi$int(16) # index number for SPECIALITY GROUND SURVEY                #
    animals$addAnimal(TYPE_GROUND, "Ground", params$DETECT_ANIMAL)              #
    TYPE_GROUNDDOG = bi$int(17) # index number for HUMAN TRACKS WITH DOG GROUND SURVEY  #
    animals$addAnimal(TYPE_GROUNDDOG, "GroundDog", params$DETECT_ANIMAL)        #
    TYPE_HANDLER = bi$int(18) # index number for DOG HANDLER TRACKS                     #
    animals$addAnimal(TYPE_HANDLER, "Handler", params$DETECT_ANIMAL)            #
    TYPE_SHOREDOG = bi$int(19) # index number for SPECIALITY ON SHORE WITH DOG          #
    animals$addAnimal(TYPE_SHOREDOG, "ShoreDog", params$DETECT_ANIMAL)          #
    TYPE_SHORELINE = bi$int(20) # index number for SPECIALITY Shoreline                 #
    ...
    myParams$setCapture(bi$int(12), bi$float(0.04), bi$float(0.01))   # note the 12 indicates LANDPLATFORM #
    myParams$setSigma(bi$int(12), bi$float(100.0), bi$float(40.0))              #
    myParams$addRRBufferAnimal(TYPE_LANDPLATFORM)                               #
    # ADD WATERPLATFORM PARAMETERS                                              #
    myParams$setCapture(bi$int(13), bi$float(0.02), bi$float(0.01))   # .008    #
    myParams$setSigma(bi$int(13), bi$float(100.0), bi$float(40.0))              #
    myParams$addRRBufferAnimal(TYPE_WATERPLATFORM)                              #
    # ADD CAMERA PARAMETERS                                                     #
    myParams$setCapture(bi$int(14), bi$float(0.06), bi$float(0.02))             #
    myParams$setSigma(bi$int(14), bi$float(100.0), bi$float(40.0))              #
    myParams$addRRBufferAnimal(TYPE_CAMERA)                                     #
    # DOGTRACK                                                                  #
    myParams$setCapture(bi$int(15), bi$float(0.14), bi$float(0.06))             #
    myParams$setSigma(bi$int(15), bi$float(100.0), bi$float(40.0))              #
    myParams$addRRBufferAnimal(TYPE_DOGTRACK)                                   #
    # GROUND                                                                    #
    myParams$setCapture(bi$int(16), bi$float(0.02), bi$float(0.01))             #
    myParams$setSigma(bi$int(16), bi$float(100.0), bi$float(40.0))              #
    myParams$addRRBufferAnimal(TYPE_GROUND)                                     #
    # GROUNDDOG                                                                 #
    myParams$setCapture(bi$int(17), bi$float(0.02), bi$float(0.01))             #
    myParams$setSigma(bi$int(17), bi$float(100.0), bi$float(40.0))              #
    myParams$addRRBufferAnimal(TYPE_GROUNDDOG)                                  #
    # HANDLER                                                                   #
    myParams$setCapture(bi$int(18), bi$float(0.02), bi$float(0.01))             #
    myParams$setSigma(bi$int(18), bi$float(100.0), bi$float(40.0))              #
    myParams$addRRBufferAnimal(TYPE_HANDLER)                                    #
    # SHOREDOG                                                                  #
    myParams$setCapture(bi$int(19), bi$float(0.02), bi$float(0.01))             #
    myParams$setSigma(bi$int(19), bi$float(100.0), bi$float(40.0))              #
    myParams$addRRBufferAnimal(TYPE_SHOREDOG)                                   #
    # SHORELINE                                                                 #
    myParams$setCapture(bi$int(20), bi$float(0.02), bi$float(0.01))             #
    myParams$setSigma(bi$int(20), bi$float(100.0), bi$float(40.0))              #
    ...
    myParams$setNumIterations(bi$int(2))
   
    ##   <<< OPTIONAL IF USING POINT SURVEILLANCE - SEE LINE 62 >>>
    myParams$setRRTrapDistance(bi$float(100.0))

    ##   <<< COMPULSORY - PERHAPS CHANGE TO SESSIONS NOT YEARS >>>
    startYear = bi$int(2015)
    endYear = bi$int(2024)
...
#    myParams.setMultipleZones(False)
    myParams$setMultipleZones(bi$bool("True"))
    ...
    startPu = bi$float(1.0)

   
    ##   <<< OPTIONAL WITH DEFAULT = 0 >>>
    ## SET THE RATE OF INCREASE OF PU
    PuIncreaseRate = bi$float(1.0)
    myParams$setPu(startPu, PuIncreaseRate)

   
    ##   <<< OPTIONAL; SET IF USE RELATIVE RISK (LINE 55) >>>
    #minimum RR value
    myParams$setMinRR(bi$float(0.5))

    ##   <<< COMPULSORY >>>
    myParams$setPrior(bi$float(0.001), bi$float(0.01), bi$float(0.1))
#    myParams.setPrior(0.005, 0.15, 0.3)

    ##   <<< COMPULSORY >>>
    myParams$setIntro(bi$float(0.00005), bi$float(0.0001), bi$float(0.0002))
    ...
    if(py_to_r(firstRun)){
        ...
        if(py_to_r(useGrids)){
            ...
            message('Grid raster FName ', py_to_r(gridSurvey))
        }
